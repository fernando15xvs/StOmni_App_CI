...
void main() {
  test('arranque usa autorización cacheada solo ante indisponibilidad de red', () {
    ...
    final offlineBranch = useCase.substring(useCase.indexOf('on SessionValidationUnavailable'));
    expect(offlineBranch, isNot(contains('invalidateSession(')));
    expect(coreFile('lib/splash/services/splash_service.dart').existsSync(), isFalse);
    expect(File('lib/splash/splash_screen.dart').readAsStringSync(), contains('restoreSession()'));
  });
  ...
  test('autorización offline tiene TTL y falla cerrado al expirar', () {
    ...
    final policy = coreFile('lib/auth/domain/session_authorization.dart').readAsStringSync();
    expect(policy, contains('offlineAuthorizationTtl = Duration(hours: 24)'));
    expect(cache, contains('CachedSessionAuthorization.offlineAuthorizationTtl'));
    expect(cache, contains('validatedAt.add(offlineAuthorizationTtl)'));
    expect(
      cache,
      contains('if (!validatedAt.add(offlineAuthorizationTtl).isAfter(now)) return null;'),
    );
    expect(cache, contains('_maxFutureClockSkew'));
    expect(cache, contains('DateTime.tryParse(validatedAtRaw)?.toUtc()'));
  });

  test('solo el gateway de validación online actualiza el snapshot offline', () {
    ...
    expect(controller, isNot(contains('AuthSessionCache.saveValidatedSession')));
  });
