...
final desktopPriceRulesProvider =
...
class DesktopPriceRulesPanel extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DesktopPriceRulesPanel> createState() =>
      _DesktopPriceRulesPanelState();
}

class _DesktopPriceRulesPanelState
    extends ConsumerState<DesktopPriceRulesPanel> {
  Future<void> _edit([SalePriceRule? current]) async {
    ...
    final presentation = TextEditingController(
      text: current?.presentationCode ?? '',
    );
    final minQuantity = TextEditingController(
      text: '${current?.minQuantity ?? 1}',
    );
    final value = TextEditingController(
      text:
          current?.fixedPrice?.toString() ??
                ...
                children: [
                  ...
                  SegmentedButton<bool>(
                    ...
                    onSelectionChanged: (set) =>
                        setDialogState(() => fixedMode = set.first),
                  ),
                  const SizedBox(height: 12),
                  TextField(
                    controller: value,
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Prioridad',
                      helperText:
                          'Si varias reglas coinciden, gana la prioridad mayor.',
                    ),
                  ),
                  SwitchListTile(
            ...
            .save(
              SalePriceRuleDraft(
                ...
                minQuantity: double.parse(
                  minQuantity.text.trim().replaceAll(',', '.'),
                ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'Reglas autoritativas aplicadas por producto, presentación y cantidad.',
                    ),
                  ],
                ),
              ),
        ...
        Expanded(
          child: rules.when(
            ...
            data: (rows) => rows.isEmpty
                ? const Center(
                    child: Text('No hay reglas de precio configuradas.'),
                  )
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        child: ListTile(
                          leading: Icon(
                            row.active ? Icons.sell : Icons.sell_outlined,
                          ),
                          title: Text(row.name),
                          subtitle: Text(
                            '${row.productName} · ${row.presentationCode ?? 'todas'} · desde ${CommercialPresentation.formatNumber(row.minQuantity)} · $rule',
                          ),
                          trailing: Wrap(
                            spacing: 4,
                            children: [
