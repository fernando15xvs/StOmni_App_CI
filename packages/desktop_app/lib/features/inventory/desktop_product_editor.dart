...
class DesktopProductEditor extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DesktopProductEditor> createState() => _DesktopProductEditorState();
}
...
class _DesktopProductEditorState extends ConsumerState<DesktopProductEditor> {
  ...
  void initState() {
    ...
    _unitPrice = TextEditingController(text: product == null ? '' : product.precioUnidad.toString());
    _packagePrice = TextEditingController(text: product?.precioCaja?.toString() ?? '0');
    _purchasePrice = TextEditingController(text: product == null ? '' : product.precioCompra.toString());
    _unitsPerPackage = TextEditingController(text: (product?.cantidadPorCaja ?? 1).toString());
    _minimumStock = TextEditingController(text: product == null ? '0' : product.stockMinimo.toString());
    ...
    try {
      ...
      final result = await useCase.execute(
        SaveProductCommand(
          ...
          warehouseIds: catalog.warehouses.map((row) => row.id).toList(growable: false),
          ...
          data: (data) => SingleChildScrollView(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Expanded(child: TextField(controller: _code, decoration: const InputDecoration(labelText: 'Código'))),
                    const SizedBox(width: 12),
                    Expanded(flex: 2, child: TextField(controller: _name, decoration: const InputDecoration(labelText: 'Nombre'))),
                  ],
                ),
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<SaleUnitType>(
                        ...
                        decoration: const InputDecoration(labelText: 'Tipo de venta legacy'),
                        items: SaleUnitType.values
                            .map((value) => DropdownMenuItem(value: value, child: Text(_saleTypeLabel(value))))
                            .toList(growable: false),
                        onChanged: _saving ? null : (value) => setState(() => _saleType = value ?? _saleType),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                ...
                Row(
                  children: [
                    Expanded(child: TextField(controller: _unitPrice, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Precio base'))),
                    const SizedBox(width: 12),
                    Expanded(child: TextField(controller: _packagePrice, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Precio base empaque'))),
                    const SizedBox(width: 12),
                    Expanded(child: TextField(controller: _purchasePrice, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Costo'))),
                  ],
                ),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(
                      child: TextField(
                        ...
                        decoration: const InputDecoration(
                          ...
                          helperText: 'Admite decimales para perfiles escalados.',
                        ),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: DropdownButtonFormField<int?>(
                        initialValue: data.suppliers.any((row) => row.id == _supplierId) ? _supplierId : null,
                        decoration: const InputDecoration(labelText: 'Proveedor'),
                        items: [
                          const DropdownMenuItem<int?>(value: null, child: Text('Sin proveedor')),
                          ...data.suppliers.map((row) => DropdownMenuItem<int?>(value: row.id, child: Text(row.name))),
                        ],
                ...
                SwitchListTile(
                  ...
                  subtitle: const Text('El stock inicial se registra después mediante Ingreso de Mercadería para conservar el contrato decimal.'),
                  value: _allowWithoutStock,
                  onChanged: _saving ? null : (value) => setState(() => _allowWithoutStock = value),
                ),
              ],
            ),
          ),
