...
class SaleLinePersistenceMapper {
  const SaleLinePersistenceMapper._();
  ...
  static SaleProcessingLine map(
    ...
    if (configuration == null && quantity != quantity.roundToDouble()) {
      throw StateError(
        'Este producto sin perfil de presentaciones no admite cantidad fraccionaria. '
    ...
    if (configuration == null) {
      if (pcs <= 0) {
        throw StateError(
          'Este producto sin perfil de presentaciones requiere una cantidad entera válida.',
        );
      }
      final canonicalBase = StockUtils.calcularCantidadBaseVenta(
        tipoVenta: tipoVenta,
        tipoUnidad: unitCode,
        cantidadVisual: quantity.toInt(),
        pcs: pcs,
      );
      baseQuantity = canonicalBase.toDouble();
      storedBaseQuantity = canonicalBase;
      storageScale = 1;
    } else {
      try {
