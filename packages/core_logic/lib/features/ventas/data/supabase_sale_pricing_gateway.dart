...
class SupabaseSalePricingGateway implements SalePricingGateway {
  ...
  Future<SalePriceQuote> resolve({
    ...
    final raw = await _client.rpc('resolve_sale_price_v1', params: {
      'p_product_id': productId,
      'p_presentation_code': presentationCode,
      'p_quantity': quantity,
      'p_at': AppTime.toIsoLima(at ?? AppTime.now()),
    });
    if (raw is! Map) throw const FormatException('Respuesta de precio inválida.');
  ...
  Future<List<SalePriceRule>> listRules() async {
    ...
    return raw.map((item) {
      if (item is! Map) throw const FormatException('Regla de precio inválida.');
  ...
  Future<int> saveRule(SalePriceRuleDraft draft, {int? id}) async {
    final raw = await _client.rpc('save_sale_price_rule_v1', params: {
      'p_id': id,
      ...
      'p_starts_at': draft.startsAt == null ? null : AppTime.toIsoLima(draft.startsAt!),
      'p_ends_at': draft.endsAt == null ? null : AppTime.toIsoLima(draft.endsAt!),
      'p_active': draft.active,
    });
    if (raw is! Map || raw['id'] is! num) {
      throw const FormatException('No se recibió el identificador de la regla.');
    }
