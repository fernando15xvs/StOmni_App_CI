...
class ProductUnitConfigurationMapper {
  ...
  static ProductUnitProfile decodeProfile(Map<String, dynamic> value) {
    ...
    final units = rows.map((row) {
      if (row is! Map ||
          ...
          CommercialPresentation.normalizeCode(row['code'] as String) != row['code']) {
      ...
      if (precision is! num || precision != precision.round()) {
        throw const FormatException('La precisión de la presentación está dañada.');
      }
      final fiscalRaw = row['fiscal_unit_code'];
      if (fiscalRaw != null && fiscalRaw is! String) {
        throw const FormatException('El código fiscal de la presentación está dañado.');
      }
      try {
        ...
        throw FormatException(error.message?.toString() ?? 'Presentación inválida.');
      }
  ...
  static Map<String, dynamic> encodeProfile(ProductUnitProfile profile) {
    ...
    return {
      ...
      'presentations': profile.presentations.map((unit) {
