...
abstract interface class PurchaseOrderGateway {
  ...
  Future<PurchaseOrderRecord> cancel(int purchaseOrderId, {required String reason});
}
...
class PurchaseOrderUseCase {
  ...
  Future<PurchaseOrderRecord> receive(ReceivePurchaseOrderCommand command) async {
    ...
    if (ids.length != command.lines.length) {
      throw ArgumentError('Una línea de compra no puede recibirse dos veces en la misma operación.');
    }
  ...
  Future<PurchaseOrderRecord> cancel(int purchaseOrderId, {required String reason}) async {
    if (purchaseOrderId <= 0 || reason.trim().isEmpty) {
      throw ArgumentError('Indica una orden y el motivo de anulación.');
    }
    final user = await _requireCapabilityAndPermission();
    final result = await _gateway.cancel(purchaseOrderId, reason: reason.trim());
    _checkSession(user);
    return result;
  }

  Future<String> _requireCapabilityAndPermission({bool receiving = false}) async {
  ...
  void _checkSession(String user) {
    if (_authorizer.currentAuthUserId != user) {
      throw const UserFacingException('La sesión cambió durante la operación de compra.');
    }
  }

  void _validateDraft(PurchaseOrderDraft draft) {
    if (draft.requestId.trim().isEmpty || draft.supplierId <= 0 || draft.warehouseId <= 0) {
      throw ArgumentError('La orden de compra no tiene proveedor, almacén o identificador válido.');
    }
    if (draft.expectedAt != null && draft.expectedAt!.isBefore(draft.orderedAt)) {
      throw ArgumentError('La fecha esperada no puede ser anterior a la fecha de la orden.');
    }
    ...
    for (final line in draft.lines) {
      ...
      if (!products.add(line.productId)) {
        throw ArgumentError('Un producto no puede repetirse en la misma orden.');
      }
    }
  }
}
