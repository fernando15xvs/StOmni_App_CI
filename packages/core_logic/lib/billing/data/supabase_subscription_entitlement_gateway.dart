...
class SupabaseSubscriptionEntitlementGateway implements SubscriptionEntitlementGateway {
  ...
  Future<List<SubscriptionEntitlement>> getCurrentEntitlements() async {
    ...
    if(raw is! List) throw const FormatException('Los entitlements son inválidos.');
    return raw.map((row)=>SubscriptionEntitlement.fromJson(Map<String,dynamic>.from(row as Map))).toList(growable:false);
  }
}
