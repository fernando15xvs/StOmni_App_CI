...
class BusinessAssistantUseCase {
  ...
  Future<BusinessAssistantSettings> updateSettings({
    ...
    if (maxResultItems < 1 ||
      ...
      throw const UserFacingException(
        'La configuración del asistente es inválida.',
      );
    }
    ...
    if (_authorizer.currentAuthUserId != user) {
      throw const UserFacingException(
        'La sesión cambió mientras se configuraba el asistente.',
      );
    }
    return saved;
  }
  ...
  Future<BusinessAssistantContext> ask({
    ...
    if (periodDays != null && (periodDays < 1 || periodDays > 365)) {
      throw const UserFacingException(
        'El periodo del asistente debe estar entre 1 y 365 días.',
      );
    }
    if (limit != null && (limit < 1 || limit > 50)) {
      throw const UserFacingException(
        'El límite de resultados del asistente es inválido.',
      );
    }
    ...
    if (_authorizer.currentAuthUserId != user) {
      throw const UserFacingException(
        'La sesión cambió mientras se consultaba el asistente.',
      );
    }
    return result;
  }
