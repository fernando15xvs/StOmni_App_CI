...
void main() {
  group('StockUtils', () {
    ...
    test('getTipoVentaFromMap mapea correctamente la base de datos', () {
      expect(
        StockUtils.getTipoVentaFromMap({'tipo_venta': 'UNIDAD'}),
        SaleUnitType.unidad,
      );
      expect(
        StockUtils.getTipoVentaFromMap({'tipo_venta': 'CAJA'}),
        SaleUnitType.caja,
      );
      expect(
        StockUtils.getTipoVentaFromMap({'tipo_venta': 'SOLO_CAJAS'}),
        SaleUnitType.caja,
      );
      expect(
        StockUtils.getTipoVentaFromMap({'tipo_venta': 'AMBOS'}),
        SaleUnitType.ambos,
      );
      expect(
        StockUtils.getTipoVentaFromMap({'tipo_venta': 'CAJA_UNIDAD'}),
        SaleUnitType.ambos,
      );
    ...
    test('formatStock formatea correctamente la UI', () {
      ...
      expect(
        StockUtils.formatStock(27, 12, SaleUnitType.ambos),
