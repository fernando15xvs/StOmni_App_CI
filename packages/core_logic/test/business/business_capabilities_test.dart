...
void main() {
  test('caché rechaza expiración y reloj futuro', () {
    ...
    expect(
      CachedBusinessProfile(profile: profile, cachedAt: now).isCurrentAt(now),
      isTrue,
    );
    expect(
  ...
  test('administrador guarda, revisión obsoleta no sobrescribe', () async {
    ...
    await expectLater(
      update.execute(original, original.capabilities),
      throwsStateError,
    );
  ...
  test('trazabilidad soportada exige lotes para vencimientos', () {
    expect(
      const BusinessCapabilities(lotTracking: true).supportedByCurrentBackend,
      isTrue,
    );
    expect(
  ...
  test('servicios no inventariables ya son una capacidad soportada', () {
    expect(
      const BusinessCapabilities(services: true).supportedByCurrentBackend,
      isTrue,
    );
  });
  ...
  test('operador no puede modificar capacidades', () async {
    ...
    await expectLater(
      update.execute(
        profiles.profile,
        const BusinessCapabilities(services: true),
      ),
  ...
  test('servidor anterior no simula persistencia de capacidades', () async {
    final profiles = TestBusinessProfiles();
    profiles.profile = const BusinessProfile(
      businessId: '1',
      displayName: 'Legacy',
      capabilities: BusinessCapabilities(),
      revision: 0,
      supportsCapabilitySettings: false,
    );
    await expectLater(
      UpdateBusinessCapabilitiesUseCase(
        gateway: profiles,
        authorizer: TestOperationAuthorizer(),
      ).execute(profiles.profile, const BusinessCapabilities()),
      throwsA(isA<UserFacingException>()),
    );
    expect(profiles.writes, 0);
  });
  ...
  test(
    'política de ventas exige capacidades sin conocer país ni comprobantes',
    () async {
      final profiles = TestBusinessProfiles();
      profiles.profile = const BusinessProfile(
        businessId: '1',
      ...
      await expectLater(
        policy.validate(isCredit: false, requiresElectronicEmission: true),
        throwsA(isA<UserFacingException>()),
      );
    },
  );
