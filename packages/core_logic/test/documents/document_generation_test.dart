...
void main() {
  ...
  test(
    'generación de ticket y cotización funciona sin assets ni plugins',
    () async {
      for (final type in TipoDocumento.values) {
        final bytes = await PdfGeneratorService.generarPdf(
          type,
          {'id': 1, 'fecha': '2026-09-04T10:00:00-05:00', 'total': 20.0},
          [],
          null,
        );
  ...
  test('reportes PDF y Excel devuelven contenido sin elegir destino', () async {
    ...
    final documents = await Future.wait([
      StockAlertPdfService.generar(
        productos: [],
        almacenes: [],
        mapaMarcas: {},
      ),
      HistorialPagosPdfService.generar(
        pagos: [],
        nombreEmpleado: 'Empleado',
        rangoFechas: 'Septiembre',
        total: 0,
      ),
      DetallePdfService.generar(
        fechaInicio: date,
        fechaFin: date,
        totalIngresos: 0,
        totalEgresos: 0,
      ...
      InventarioPdfService.generar(
        fechaInicio: date,
        fechaFin: date,
        movimientos: [],
        incluirIngresos: true,
        incluirSalidas: true,
        incluirTraslados: true,
      ),
      AlmacenExcelService.generar(
        configalmacenes: [],
        productosCompletos: [],
        mapaMarcas: {},
      ),
      ReporteExcelService.generar(
        fechaInicio: date,
        fechaFin: date,
        snapshot: const ReportesExcelSnapshot(),
        incluirVentas: true,
        incluirAbonos: false,
        incluirGastos: false,
        incluirPersonal: false,
      ),
    ]);
