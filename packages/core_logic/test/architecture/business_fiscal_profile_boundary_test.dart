...
void main() {
  test('el perfil fiscal es neutral y no depende de proveedor, UI o Supabase', () {
    final source = _read(
  ...
  test('ConfiguracionService resuelve el perfil por tenant y no por configId de UI', () {
    final source = _read('lib/services/configuracion_service.dart');

    expect(source, contains('BusinessFiscalProfile? get fiscalProfile'));
    expect(source, contains('BusinessFiscalProfileKey get activeFiscalProfileKey'));
    expect(source, contains('selectFiscalProfile(BusinessFiscalProfileKey key)'));
  ...
  test('la pantalla de configuración consume el perfil tipado y no el mapa legacy', () {
    final source = _read(
    ...
    expect(source, isNot(contains('ConfiguracionService.actualizarNegocio({')));
    expect(source, isNot(contains("data['razon_social']")));
  });

  test('documentos existentes consumen perfil fiscal y no identidad sectorial legacy', () {
