...
void main() {
  ProductUnitProfile profile() {
    ...
    return ProductUnitProfile(baseUnit: bottle, presentations: [
      bottle,
      CommercialPresentation(code: 'pack_6', singularLabel: 'Pack',
          pluralLabel: 'Packs', baseQuantity: 6),
      CommercialPresentation(code: 'caja_24', singularLabel: 'Caja',
          pluralLabel: 'Cajas', baseQuantity: 24),
    ]);
  }
  ...
  test('rechaza perfiles fraccionarios hasta migrar todo el inventario', () {
    final kg = CommercialPresentation.base(code: 'kg', singularLabel: 'kg',
        pluralLabel: 'kg', allowsFractionalSale: true);
    final value = ProductUnitProfile(baseUnit: kg, presentations: [kg]);
    expect(() => IntegerPresentationPolicy.validate(value), throwsArgumentError);
  });
  ...
  test('codec conserva revisión, etiquetas y equivalencias', () {
    ...
    expect(() => decoded.profile.presentations.add(decoded.profile.baseUnit),
        throwsUnsupportedError);
  });

  test('precios y línea persistible usan el perfil versionado', () {
    final configuration = ProductUnitConfiguration(revision: 3, profile: profile());
    final product = SaleProductSnapshot(
      name: 'Agua', saleType: SaleUnitType.cajaUnidades,
      unitsPerPackage: 24, defaultUnitPrice: 2,
      defaultPackageBasePrice: 2, unitConfiguration: configuration,
    );
    final priced = const PriceSaleLineUseCase().execute(
      SaleCartLine(productId: 5, warehouseId: 2, product: product,
          quantity: 2, commercialUnit: 'pack_6', subtotal: 0),
  ...
  test('snapshot de carrito conserva la configuración para la cola offline', () {
    final configuration = ProductUnitConfiguration(revision: 2, profile: profile());
    final cart = SaleCart([
      SaleCartLine(productId: 8, warehouseId: 1, quantity: 1,
          commercialUnit: 'caja_24', subtotal: 40,
          product: SaleProductSnapshot(name: 'Agua',
  ...
  test('codec de producto mantiene configuración e inventario inmutables', () {
    ...
    expect(restored.unitConfiguration!.profile.find('caja_24')!.baseQuantity, 24);
  ...
  test('guardado exige permiso y revisión, y bloquea cambiar la base', () async {
    final gateway = _Gateway(ProductUnitConfiguration(revision: 2, profile: profile()));
    final current = ProductUnitSettings(supported: true, configuration: gateway.value);
    ...
    final changedBase = CommercialPresentation.base(
      code: 'unidad', singularLabel: 'Unidad', pluralLabel: 'Unidades');
    ...
    await expectLater(
      useCase.execute(
        productId: 1,
        current: current,
        profile: ProductUnitProfile(baseUnit: changedBase, presentations: [changedBase]),
      ),
      throwsA(isA<UserFacingException>()),
    );
    expect(gateway.writes, 0);
...
class _Gateway implements ProductUnitConfigurationGateway {
  ...
  @override
  Future<ProductUnitConfiguration> save({required int productId,
      required int expectedRevision, required ProductUnitProfile profile}) async {
    if (expectedRevision != value.revision) throw StateError('Revisión obsoleta');
    writes++;
