...
void main() {
  test(
    'producto rechaza importes no finitos y equivalencias inválidas antes de efectos',
    () async {
      final data = _NoEffects();
      final useCase = SaveProductUseCase(
        inventory: data,
        productAdmin: data,
        uuid: const Uuid(),
      ...
      for (final command in [
        _command(price: double.nan),
        _command(price: double.infinity),
        _command(pcs: 0),
      ]) {
        await expectLater(
  ...
  test(
    'operador no crea productos aunque invoque directamente el caso de uso',
    () async {
      final data = _NoEffects();
      final useCase = SaveProductUseCase(
        inventory: data,
...
SaveProductCommand _command({double price = 10, int pcs = 1}) =>
    SaveProductCommand(
      requestId: 'request-1',
      productId: null,
      code: 'P1',
      ...
      purchasePrice: 0,
      unitsPerPackage: pcs,
      saleUnitType: SaleUnitType.unidad,
      minimumStock: 0,
      supplierId: null,
