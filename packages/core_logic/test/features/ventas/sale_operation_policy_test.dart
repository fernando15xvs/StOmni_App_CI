...
void main() {
  test('sin permiso no resuelve clientes ni persiste', () async {
    ...
    final useCase = _useCase(
      data,
      TestOperationAuthorizer(role: 'sin_rol'),
      TestBusinessProfiles(),
    );
    await expectLater(
      useCase.ejecutarCommand(_command()),
  ...
  test('capacidades bloquean venta online antes de cualquier efecto', () async {
    ...
    await expectLater(
      useCase.ejecutarCommand(_command(credit: true)),
      throwsA(isA<UserFacingException>()),
    );
    expect(data.effects, 0);
  });
  ...
  test(
    'offline usa la misma autorización y capacidades antes de encolar',
    () async {
      final data = _Data();
      ...
      final request = VentaSubmissionRequest(
        ...
        motivoDescuento: '',
        cliente: command.customer,
        pagos: command.payments,
        carrito: command.cart,
      );
      await expectLater(
...
TestBusinessProfiles _noCredit() =>
    TestBusinessProfiles()
      ..profile = const BusinessProfile(
        businessId: '1',
        displayName: 'Pruebas',
        capabilities: BusinessCapabilities(creditSales: false),
        revision: 1,
        supportsCapabilitySettings: true,
      );

ProcesarVentaUseCase _useCase(
  _Data data,
  TestOperationAuthorizer authorizer,
  TestBusinessProfiles profiles,
) => ProcesarVentaUseCase(
  context: data,
  processing: data,
  authorizer: authorizer,
  businessPolicy: ConfiguredBusinessSalePolicy(profiles),
...
class _Data implements VentaContextGateway, VentaProcessingGateway {
  ...
  Future<VentaProcessingResult> processSale(
    ...
    effects++;
    return const VentaProcessingResult(idempotent: false);
  }
}
