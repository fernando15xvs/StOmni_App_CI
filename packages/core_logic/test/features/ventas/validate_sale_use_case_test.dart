...
void main() {
  ...
  test('rechaza números no finitos, total inconsistente y pagos incompletos', () {
  ...
  test('venta a crédito exige que los pagos coincidan con el abono', () {
    expect(validator.execute(_command(credit: true, deposit: 5, paid: 5)).subtotal, 20);
    expect(() => validator.execute(_command(credit: true, deposit: 5, paid: 4)), throwsStateError);
    expect(validator.execute(_command(credit: true, deposit: 0, paid: 0)).subtotal, 20);
  });
  test('policy fiscal sustituible y validación previa a efectos de datos', () async {
    ...
    final useCase = ProcesarVentaUseCase(
      ...
      context: context, processing: processing, fiscalPolicy: const _ReceiptPolicy(),
    );
    await expectLater(useCase.ejecutarCommand(_command(type: 'rechazado')), throwsStateError);
  ...
  test('no persiste si cambia la sesión al resolver el cliente', () async {
    ...
    final useCase = ProcesarVentaUseCase(
      authorizer: TestOperationAuthorizer(user: 'user-1'),
      businessPolicy: ConfiguredBusinessSalePolicy(TestBusinessProfiles()),
      context: context, processing: processing,
      fiscalPolicy: const InternalTicketFiscalPolicy(),
    );
    await expectLater(useCase.ejecutarCommand(_command()), throwsStateError);
    expect(processing.request, isNull);
  });
  test('presentaciones configuradas bloquean emisión electrónica antes de efectos', () {
    final context = _Context();
    final useCase = ProcesarVentaUseCase(
      authorizer: TestOperationAuthorizer(user: 'user-1'),
      businessPolicy: ConfiguredBusinessSalePolicy(TestBusinessProfiles()),
    ...
    final base = CommercialPresentation.base(
      code: 'botella', singularLabel: 'Botella', pluralLabel: 'Botellas');
    final profile = ProductUnitProfile(baseUnit: base, presentations: [base]);
    final product = SaleProductSnapshot(
      name: 'Agua',
      unitConfiguration: ProductUnitConfiguration(revision: 1, profile: profile),
    );
    final command = ProcesarVentaCommand(
      requestId: 'configured-electronic', fecha: DateTime(2026, 9, 4),
      tipoComprobante: 'electronic',
      customer: const SaleCustomer(ruc: '', nombre: 'Cliente', direccion: ''),
      esCredito: false, totalAPagar: 20, montoAbono: 0, subtotalBruto: 20,
      payments: const [SalePayment(metodo: 'Efectivo', monto: 20)],
      cart: SaleCart([SaleCartLine(productId: 1, warehouseId: 1,
        product: product, quantity: 2, subtotal: 20,
        commercialUnit: 'botella', commercialUnitPrice: 10,
        baseUnitPrice: 10, recordedBaseQuantity: 2)]),
    );
...
ProcesarVentaCommand _command({
  double total = 20, double paid = 20, double deposit = 0,
  double lineSubtotal = 20, bool credit = false, String type = 'ticket_interno',
}) => ProcesarVentaCommand(
  requestId: 'sale-1', fecha: DateTime(2026, 9, 4), tipoComprobante: type,
  customer: const SaleCustomer(ruc: '', nombre: 'Cliente', direccion: ''),
  esCredito: credit, totalAPagar: total, montoAbono: deposit, subtotalBruto: 20,
  payments: paid == 0 ? [] : [SalePayment(metodo: 'Efectivo', monto: paid)],
  cart: SaleCart([
    SaleCartLine(productId: 1, quantity: 2, subtotal: lineSubtotal,
      commercialUnit: 'unidad', warehouseId: 3, commercialUnitPrice: 10,
      baseUnitPrice: 10, recordedBaseQuantity: 2),
  ]),
);
...
class _ReceiptPolicy implements FiscalPolicy {
  ...
  FiscalDocumentType? documentFor(String code) => code == 'receipt'
      ? const FiscalDocumentType(code: 'receipt', requiresElectronicEmission: false, allowsOffline: false)
      : null;
  @override
  String? validate(FiscalSaleDraft sale, {required DateTime now}) =>
      documentFor(sale.documentCode) == null ? 'Documento no habilitado.' : null;
}
...
class _ElectronicPolicy implements FiscalPolicy {
  ...
  FiscalDocumentType? documentFor(String code) => code == 'electronic'
      ? const FiscalDocumentType(
          code: 'electronic', requiresElectronicEmission: true, allowsOffline: false)
      : null;
  @override
...
class _Processing implements VentaProcessingGateway {
  ...
  Future<VentaProcessingResult> processSale(VentaProcessingRequest request) async {
    ...
    return const VentaProcessingResult(idempotent: false, comprobanteId: 'sale-1');
  }
}
