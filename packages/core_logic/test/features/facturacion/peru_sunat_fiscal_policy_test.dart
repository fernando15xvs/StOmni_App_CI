...
void main() {
  ...
  String? validate({String type = 'boleta', double total = 700,
    String document = '', String name = '', bool credit = false, DateTime? date}) =>
      policy.validate(FiscalSaleDraft(
        documentCode: type,
        date: date ?? now,
        isCredit: credit,
        total: total,
  ...
  test('se conserva el umbral existente de identificación para boleta', () {
    ...
    expect(validate(total: 701, document: '12345678'), contains('nombre completo'));
  ...
  test('se conservan crédito, fecha y datos de factura', () {
    ...
    expect(validate(date: now.add(const Duration(days: 1))), contains('futuro'));
    expect(validate(type: 'factura', document: '20123456789', name: 'Empresa',
      date: now.subtract(const Duration(days: 3))), isNull);
    expect(validate(type: 'factura', document: '20123456789', name: 'Empresa',
      date: now.subtract(const Duration(days: 4))), contains('tres días'));
  ...
  test('política interna no hereda condiciones de emisión electrónica', () {
    ...
    expect(internal.validate(FiscalSaleDraft(
      documentCode: 'ticket_interno', date: DateTime(2000), isCredit: true,
      total: 1500, customer: const SaleCustomer(ruc: '', nombre: '', direccion: ''),
    ), now: now), isNull);
  });
}
